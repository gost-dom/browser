...
const from = 'idiomorph';
const generatedByIdiomorphId = new WeakSet();
//=============================================================================
// Core Morphing Algorithm - morph, morphNormalizedContent, morphOldNodeTo, morphChildren
//=============================================================================
export function idiomorph(oldNode, newContent, config = {}) {
    if (oldNode instanceof Document) {
        oldNode = oldNode.documentElement;
    }
    let parsedContent;
    if (typeof newContent === 'string') {
        parsedContent = parseContent(newContent);
    }
    else {
...
function morphNormalizedContent(oldNode, normalizedNewContent, ctx) {
    ...
    if (ctx.morphStyle === 'outerHTML' || ctx.morphStyle == null) {
        ...
        if (!bestMatch) {
            throw internalErr(from, 'NoBestMatchFound', {
                old: oldNode,
    ...
    throw internalErr(from, 'InvalidMorphStyle', { style: ctx.morphStyle });
}
/**
...
function morphOldNodeTo(oldNode, newContent, ctx) {
    if (ctx.ignoreActive && oldNode === document.activeElement) {
        ...
        else if (newContent instanceof HTMLHeadElement &&
...
function isIdSetMatch(node1, node2, ctx) {
    ...
    if (node1.nodeType === node2.nodeType && node1.tagName === node2.tagName) {
        if (node1?.id?.length && node1.id === node2.id)
            return true;
...
function removeNodesBetween(startInclusive, endExclusive, ctx) {
    while (startInclusive !== endExclusive) {
        ...
        if (!tempNode) {
            throw internalErr(from, 'NoTemporaryNodeFound', {
                startInclusive,
                endExclusive,
            });
        }
...
function parseContent(newContent) {
    ...
    if (!content) {
        throw internalErr(from, 'NoContentFound', { newContent });
    }
    generatedByIdiomorphId.add(content);
    return content;
}
...
function findBestNodeMatch(newContent, oldNode, ctx) {
    let currentElement = newContent.firstChild;
